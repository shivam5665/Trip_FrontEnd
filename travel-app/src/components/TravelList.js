...
const TravelList = () => {
  ...
  const fetchTravels = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/travel');
  ...
  const handleFilterDestination = async (e) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3001/api/travel?destination=${selectedDestination}`);
      setTravels(response.data);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleSortBudget = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/travel?sort=budget');
      setTravels(response.data);
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div style={{'margin':'auto','border':'2px solid blue','width':'500px','marginTop':'70px','textAlign':'center'}}>
      <h2>Retrieve Data</h2>
      <div>
        <label>
